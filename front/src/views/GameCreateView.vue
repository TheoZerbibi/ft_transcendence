...
export default {
	...
	methods: {
		checkExistingGame: async function () {
			...
			try {
				const response = await fetch(`http://${VITE_HOST}:${VITE_API_PORT}/game/getEmptyGame`, requestOptions);
		...
		createGame: async function () {
			...
			try {
				const response = await fetch(`http://${VITE_HOST}:${VITE_API_PORT}/game/createGame`, requestOptions);
				snackbarStore.hideSnackbar();
				if (!response.ok) {
